// Client-side RPE table (mirrors utils/rpe.js)
...
export function snapRpe(rpe) {
  return VALID_RPES.reduce((prev, curr) =>
    Math.abs(curr - rpe) < Math.abs(prev - rpe) ? curr : prev
  );
}

export function getPercentage(reps, rpe) {
  const r = Math.min(10, Math.max(1, Math.round(reps)));
  const snapped = snapRpe(Number(rpe));
  return (RPE_TABLE[r]?.[snapped] ?? 70) / 100;
}

export function calcE1RM(load, reps, rpe) {
  const pct = getPercentage(reps, rpe);
  return pct > 0 ? load / pct : null;
}

export function roundToNearest(value, increment = 2.5) {
  return Math.round(value / increment) * increment;
}

export function calcBackdownLoad(e1rm, backdownReps, targetRpe, increment = 2.5) {
  const pct = getPercentage(backdownReps, targetRpe);
  return roundToNearest(e1rm * pct, increment);
}

export function calcIntensityPct(load, e1rm) {
  if (!e1rm || e1rm === 0) return null;
  return Math.round((load / e1rm) * 1000) / 10;
}

export function rpeClass(rpe) {
  if (!rpe) return '';
  if (rpe <= 7)  return 'rpe-low';
  if (rpe <= 8.5) return 'rpe-medium';
  return 'rpe-high';
}
